...
(function(){
  ...
  function buildReportHtml(d){
    ...
    *{box-sizing:border-box}body{font-family:Arial,sans-serif;color:#24130f;margin:0;padding:24px;background:#fff}.head{background:#2a0d08;color:#fff;padding:22px;border-bottom:6px solid #a81414}.head h1{margin:0 0 4px;font-size:24px}.head p{margin:3px 0}.period{margin:18px 0;font-size:15px;font-weight:700}.kpis{display:grid;grid-template-columns:repeat(5,1fr);gap:10px;margin:14px 0}.kpi{border:1px solid #e4d5d0;border-radius:10px;padding:12px}.kpi small{display:block;color:#765b52;margin-bottom:5px}.kpi strong{font-size:19px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin:18px 0}.box{border:1px solid #e4d5d0;border-radius:10px;padding:14px}.box h3{margin-top:0}.line{display:flex;justify-content:space-between;border-bottom:1px solid #eee;padding:7px 0}table{width:100%;border-collapse:collapse;margin-top:15px;font-size:11px}th,td{border:1px solid #ddd;padding:7px;text-align:left}th{background:#f3ece9}.footer{margin-top:20px;font-size:10px;color:#777}@media print{body{padding:0}.head{-webkit-print-color-adjust:exact;print-color-adjust:exact}.no-print{display:none}.kpis{grid-template-columns:repeat(5,1fr)}}
    </style></head><body><div class="head"><h1>ATACADÃO DA CARNE O PEITICA</h1><p>Balanço de vendas do aplicativo</p><p>Rua Edison Martins, 530 • Fortaleza</p></div><div class="period">Período: ${esc(periodText())}</div><div class="kpis"><div class="kpi"><small>Pedidos</small><strong>${k.pedidos||0}</strong></div><div class="kpi"><small>Faturamento</small><strong>${fmtMoney(k.faturamento)}</strong></div><div class="kpi"><small>Ticket médio</small><strong>${fmtMoney(k.ticket_medio)}</strong></div><div class="kpi"><small>Entregas</small><strong>${k.entregas||0}</strong></div><div class="kpi"><small>Retiradas</small><strong>${k.retiradas||0}</strong></div></div><div class="grid"><div class="box"><h3>Produtos mais vendidos</h3>${top.map((x,i)=>`<div class="line"><span>${i+1}. ${esc(x.nome)}</span><strong>${fmtMoney(x.faturamento)}</strong></div>`).join('')||'<p>Sem vendas.</p>'}</div><div class="box"><h3>Faturamento por bairro</h3>${bairros.map(x=>`<div class="line"><span>${esc(x.bairro)}</span><strong>${fmtMoney(x.faturamento)}</strong></div>`).join('')||'<p>Sem entregas.</p>'}</div></div><h3>Pedidos do período</h3><table><thead><tr><th>Pedido</th><th>Data</th><th>Cliente</th><th>Tipo</th><th>Bairro</th><th>Total</th><th>Status</th></tr></thead><tbody>${rows||'<tr><td colspan="7">Nenhum pedido no período.</td></tr>'}</tbody></table><div class="footer">Relatório gerado em ${new Date().toLocaleString('pt-BR')} • O Peitica Gestão</div><script>window.onload=()=>setTimeout(()=>window.print(),300)<\/script></body></html>`;
  }
