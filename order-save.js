(function(){
  ...
  function getPayload(){
    ...
    return {
      ...
      localizacao:deliveryMapLinkFromCustomer(c),
      ...
      itens:state.cart.map(i=>({
        ...
        preco_unitario:i.preco,subtotal:i.total,observacao:i.note||''
      }))
    };
  }
  ...
  function saveLocal(payload){
    try{
      const all=JSON.parse(localStorage.getItem(LOCAL_KEY)||'[]').filter(x=>x.id!==payload.id);
      all.unshift(payload);
  ...
  async function saveToSheet(payload){
    ...
    try{
      ...
      await fetch(cfg.ORDER_WEBAPP_URL,{method:'POST',body,mode:'no-cors'});
      return {ok:true};
  ...
  function makeBrandedPdf(payload){
    ...
    if(logo){try{doc.addImage(logo,'JPEG',9,5,32,32);}catch(e){}}
    doc.setTextColor(255,255,255);doc.setFont('helvetica','bold');doc.setFontSize(17);
    doc.text('ATACADÃO DA CARNE O PEITICA',46,13);
    doc.setFont('helvetica','normal');doc.setFontSize(9);
    doc.text('Rua Edison Martins, 530 - Fortaleza/CE',46,20);
    doc.text('WhatsApp: (85) 98962-6829',46,26);
    doc.text(`ORÇAMENTO ${payload.id}`,46,32);
    doc.text(created,196,32,{align:'right'});

    let y=51;
    doc.setTextColor(20,20,20);doc.setFont('helvetica','bold');doc.setFontSize(12);
    doc.text('DADOS DO CLIENTE',14,y);y+=7;
    doc.setFont('helvetica','normal');doc.setFontSize(10);
    doc.text(`Nome: ${payload.cliente}`,14,y);y+=6;
    doc.text(`Telefone/WhatsApp: ${payload.telefone}`,14,y);y+=6;
    doc.text(`Recebimento: ${payload.recebimento==='entrega'?'ENTREGA':'RETIRADA NA LOJA'}`,14,y);y+=8;
    ...
    if(payload.recebimento==='entrega'){
      ...
      doc.text(`Bairro: ${payload.bairro}`,14,y);
      doc.text(`Taxa de entrega: ${money(payload.taxa_entrega)}`,115,y);y+=6;
      ...
      if(payload.localizacao){
        doc.setFillColor(246,238,237);doc.roundedRect(14,y-2,182,16,2,2,'F');
        doc.setTextColor(153,0,0);doc.setFont('helvetica','bold');doc.setFontSize(10);
        doc.textWithLink('ABRIR ROTA DA ENTREGA NO GOOGLE MAPS',18,y+4,{url:payload.localizacao});
        doc.setFont('helvetica','normal');doc.setFontSize(7.5);
        const linkLines=doc.splitTextToSize(payload.localizacao,166);doc.text(linkLines,18,y+9);
        doc.setTextColor(20,20,20);doc.setFontSize(10);y+=19;
      }
    }else{
      doc.text('Retirada: Rua Edison Martins, 530 - Fortaleza/CE',14,y);y+=8;
    }

    doc.setDrawColor(210);doc.line(14,y,196,y);y+=8;
    doc.setFont('helvetica','bold');doc.setFontSize(12);doc.text('PRODUTOS DO ORÇAMENTO',14,y);y+=7;

    payload.itens.forEach((i,n)=>{
      if(y>252){doc.addPage();y=20;}
      doc.setFont('helvetica','bold');doc.setFontSize(10);doc.text(`${n+1}. ${i.nome}`,14,y,{maxWidth:92});
      doc.setFont('helvetica','normal');
      const qtd=i.unidade==='kit'?`${i.quantidade} kit(s)`:`${i.quantidade} g`;
      doc.text(qtd,120,y);doc.text(money(i.subtotal),196,y,{align:'right'});y+=5;
      doc.setFontSize(8.5);doc.setTextColor(85);
      doc.text(`Categoria: ${i.categoria||'-'} | Preço: ${money(i.preco_unitario)} ${i.unidade==='kit'?'/ kit':'/ kg'}`,18,y,{maxWidth:170});y+=5;
    ...
    doc.text('Documento para separação, conferência, impressão e entrega.',14,y);
    doc.text(`Pedido: ${payload.id}`,196,y,{align:'right'});
  ...
  function clearCurrentBudget(){
    ...
    try{
      state.cart=[];state.location=null;
  ...
  async function finalizeBudget(){
    ...
    if(btn){btn.disabled=true;btn.textContent='⏳ GERANDO PDF COMPLETO...';}
    ...
    try{
      ...
      if(canShareFiles){
        await navigator.share({title:`Orçamento ${payload.id} - O Peitica`,files:[file]});
        await sheetPromise;
        clearCurrentBudget();
      }else{
        ...
        openPdf(blob);
        await sheetPromise;
        setTimeout(()=>window.open(cfg.WHATSAPP_BUSINESS_URL||'https://wa.me/message/NW4MC6V5OVEIK1','_blank','noopener'),700);
        clearCurrentBudget();
        alert('PDF completo gerado. Ele foi baixado e aberto para impressão. No computador, o WhatsApp Web não permite anexar o PDF automaticamente; a conversa será aberta para você anexar o arquivo PDF já baixado.');
      }
    }catch(e){
      if(e?.name!=='AbortError'){
        downloadBlob(blob,filename);openPdf(blob);
        alert('O PDF foi gerado. O compartilhamento automático não foi permitido por este navegador, então o arquivo foi aberto e baixado para impressão/encaminhamento.');
      }
    }finally{
